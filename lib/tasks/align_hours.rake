namespace :redmine do
  namespace :done_ratio_via_time do
    ...
    task :align_hours => :environment do
      ...
      project_scope = Project.active.joins(:enabled_modules).where("enabled_modules.name = 'issue_progress'").select(:id)
      Issue.where(project_id: project_scope).where(status_id: DoneRatioSetup.settings[:global][:statuses_for_hours_alignment].to_a).includes(:time_entries).find_each do |issue|
